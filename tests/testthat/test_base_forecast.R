...
test_that("forecast with regular tasks", {
  ...
  expect_warning((cm2 = train(makeLearner("classif.lda"), multiclass.task.lag, subset = inds)))
  ...
  expect_warning((cm3 = train(wl.lda, multiclass.task.lag, subset = inds)))
  ...
  expect_warning((cm5 = train(wl.lda, binaryclass.task.lag, subset = binaryclass.train.inds.lag)))
...
test_that("forecast works with strange class labels", {
  ...
  expect_warning((mod = train(makeLearner("classif.lda", predict.type = "prob"), task = task.lag)))
  p = forecast(mod, h = binaryclass.h, newdata = df[binaryclass.test.inds,,drop=FALSE])
  expect_equal(colnames(p$data), c( "truth", "prob.-1", "prob.1", "prob.response", "response"))
})
